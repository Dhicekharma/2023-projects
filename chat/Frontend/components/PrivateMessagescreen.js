...
import { privatesocket } from "../utils/index";
import * as DocumentPicker from 'expo-document-picker';
import * as FileSystem from 'expo-file-system';
import { FontAwesome5 } from '@expo/vector-icons'; // Import FontAwesome5 icons

export default function PrivateMessagescreen({ navigation, route }) {
  ...
  function handleAddNewMessage() {
    ...
    if (currentUser) {
      privatesocket.emit("newChatMessage", {
        currentChatMesage,
  ...
  useEffect(() => {
    privatesocket.emit('findGroup', currentGroupID);
    privatesocket.on('foundGroup', (allChats) => setAllChatMessages(allChats));
  }, [privatesocket]);
  ...
  async function handleFileUpload() {
    try {
      ...
      if (result.type === 'success') {
        ...
        privatesocket.emit('newFileMessage', {
